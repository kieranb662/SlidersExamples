...
struct HueSliderStyle: LSliderStyle {
    ...
    func makeTrack(configuration: LSliderConfiguration) -> some View {
        GeometryReader { proxy in
            ...
            .frame(width: proxy.size.width + strokeWidth)
            .offset(x: -strokeWidth/2)
        }
    }
}

struct SaturationBrightnessStyle: TrackPadStyle {
    var hue: Double
...
struct HSBColorPicker: View {
    ...
    var body: some View {
        let brightnessSaturation = Binding(
            get: { CGPoint(x: saturation, y: brightness) },
            set: { (new) in
                saturation = Double(new.x)
                brightness = Double(new.y)
            })
        
        return VStack(spacing: 30) {
            TrackPad(value: brightnessSaturation, rangeX: 0.01...1, rangeY: 0.01...1)
                .trackPadStyle(SaturationBrightnessStyle(hue: hue))
